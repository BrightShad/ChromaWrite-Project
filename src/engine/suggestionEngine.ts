import type { PrimaryEmotion, Suggestion, SuggestionTier } from '../types/emotion'

// ─── Tier 1 Templates ─────────────────────────────────────────────────────────
// Free. Fires when flatlineCount >= FLATLINE_TRIGGER.
// Never calls API.

const FLATLINE_TRIGGER = 5  // consecutive cycles in same emotion

const TIER1_TEMPLATES: Record<PrimaryEmotion, string[]> = {
  Melancholy: [
  ...
  Joy: [
    ...
    "Consider a moment that complicates the brightness — not negates it, just adds texture.",
  ],
  Grief: [
...
// ─── Tier 3 Templates ─────────────────────────────────────────────────────────
// Free. Fires on conflict detection (two or more emotions blending).
...
let _id = 0
function uid() { return `sug_${Date.now()}_${_id++}` }

// ─── Suggestion Engine ────────────────────────────────────────────────────────

export class SuggestionEngine {
  private lastTier1At: number = -99  // cycle index of last Tier 1 fire
  private lastTier3At: number = -99

  /** Minimum cycles between Tier 1 fires */
  private readonly TIER1_COOLDOWN = 8
  /** Minimum cycles between Tier 3 fires */
  private readonly TIER3_COOLDOWN = 4

  /**
   * Evaluate whether a suggestion should fire this cycle.
   * Call after each arc.record().
  ...
  evaluate(params: {
    ...
    prompterCharge: number  // 0–100 — Tier 2 only fires if charged
  }): Suggestion | null {
    ...
    if (
      ...
      const templates = TIER1_TEMPLATES[dominant]
  ...
  /**
   * Check if a Tier 2 (API) nudge is available.
   * Does NOT fire it — just signals availability to the UI.
   * Actual fire happens via apiClient.getNudge().
   */
  tier2Available(prompterCharge: number): boolean {
