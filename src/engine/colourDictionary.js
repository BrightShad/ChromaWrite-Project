...
function entry(hex, accent, transitionMs, label) {
  return {
...
export const COLOUR_DICTIONARY = {
  // --- Anger Family ---
 ...
 */
export function blendColours(emotions) {
 ...
 */
export function confidenceToSaturation(confidence) {
 ...
 */
export function buildCSSVars(dominant, blendEmotions, scores, confidence) {
  const saturation = confidenceToSaturation(confidence);
  const entryVal = COLOUR_DICTIONARY[dominant];
  ...
  return {
    '--cw-emotion-primary':   entryVal.primary.hex,
    '--cw-emotion-accent':    entryVal.accent,
    '--cw-emotion-blended':   blendedHex,
    '--cw-emotion-rgb':       `${br}, ${bg}, ${bb}`,
    '--cw-emotion-saturation': `${Math.round(saturation * 100)}%`,
    '--cw-transition-ms':     `${entryVal.transitionMs}ms`,
  };
}
